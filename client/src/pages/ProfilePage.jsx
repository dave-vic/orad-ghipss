...
import {
  User, SlidersHorizontal, Bell, Link2, Lock, Monitor, AlertTriangle, Camera,
} from 'lucide-react';
...
function EmailForm({ current, onSaved, onCancel }) {
  const [email, setEmail] = useState(current);
  const [saving, setSaving] = useState(false);
  const [err, setErr] = useState('');
  const inp = { width: '100%', padding: '8px 12px', border: '1.5px solid #306196', borderRadius: '6px', fontSize: '13px', color: '#112235', outline: 'none', boxSizing: 'border-box', marginBottom: '10px' };
  const save = async () => {
    if (!email || !email.includes('@')) { setErr('Enter a valid email address'); return; }
    setSaving(true); setErr('');
    try {
      await api.patch('/auth/profile', { email });
      onSaved('Email updated successfully');
    } catch (e) { setErr(e.response?.data?.error || 'Failed to update email'); }
    finally { setSaving(false); }
  };
  return (
    <div style={{ marginTop: '8px', padding: '14px 16px', backgroundColor: '#F7F8FA', borderRadius: '8px', border: '1px solid #E5E7EB' }}>
      <div style={{ fontSize: '12px', fontWeight: '600', color: '#6B7280', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: '8px' }}>New email address</div>
      <input type="email" value={email} onChange={e => setEmail(e.target.value)} style={inp}
        onKeyDown={e => e.key === 'Enter' && save()} autoFocus />
      {err && <div style={{ fontSize: '12px', color: '#DC2626', marginBottom: '8px' }}>{err}</div>}
      <div style={{ display: 'flex', gap: '8px' }}>
        <button onClick={save} disabled={saving}
          style={{ padding: '7px 16px', fontSize: '13px', fontWeight: '600', backgroundColor: '#306196', color: '#FFFFFF', border: 'none', borderRadius: '7px', cursor: 'pointer', opacity: saving ? 0.6 : 1 }}>
          {saving ? 'Saving…' : 'Save'}
        </button>
        <button onClick={onCancel}
          style={{ padding: '7px 14px', fontSize: '13px', fontWeight: '600', backgroundColor: '#FFFFFF', border: '1px solid #D1D5DB', borderRadius: '7px', color: '#374151', cursor: 'pointer' }}>
          Cancel
        </button>
      </div>
    </div>
...
/* ─── Sections ─── */
function SectionProfile({ data, onRefresh, onToast, goToSecurity }) {
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '20px', marginBottom: '28px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
          <div style={{ position: 'relative', cursor: 'pointer' }} onClick={() => fileRef.current?.click()}>
            {photoSrc
              ? <img src={photoSrc} alt="avatar" style={{ width: 56, height: 56, borderRadius: '50%', objectFit: 'cover', border: '2px solid #E5E7EB' }} />
              : <Avatar name={data?.name} role={data?.role} size={56} />
            }
            <div style={{ position: 'absolute', inset: 0, borderRadius: '50%', backgroundColor: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0, transition: 'opacity 0.15s' }}
          ...
          <button onClick={() => fileRef.current?.click()}
            style={{ fontSize: '12px', color: '#306196', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
            Change photo
          </button>
        </div>
        <div style={{ flex: 1 }}>
          ...
          <input value={name} onChange={e => setName(e.target.value)}
            ...
            onBlur={e => { e.target.style.borderColor = '#D1D5DB'; saveName(); }}
          />
      ...
      <SecRow
        label="Email"
        desc={data?.email}
        action={ghostAction(showEmail ? 'Cancel' : 'Manage emails', () => setShowEmail(v => !v))}
      />
      {showEmail && (
        <EmailForm
          current={data?.email || ''}
          onSaved={(msg) => { onToast(msg); setShowEmail(false); onRefresh(); }}
          onCancel={() => setShowEmail(false)}
        />
      )}
      <SecRow label="Password" desc="Set a password for your account" action={ghostAction('Change password', goToSecurity)} />
      <SecRow label="Two-step verification" desc="Add another layer of security to your account" action={ghostAction('Set up 2FA', goToSecurity)} />
      <SecRow label="Passkeys" desc="Sign in with on-device biometric authentication"
        action={ghostAction('Add passkey', () => onToast('Passkey support coming soon'))} last />
      ...
      <Divider />
      <SecRow label="Delete my account" desc="Permanently delete your account. You will lose all access immediately." action={dangerAction('Delete my account')} last />
    </>
  );
}
...
function SectionPreferences() {
  ...
  const sel = { padding: '7px 10px', fontSize: '13px', border: '1px solid #D1D5DB', borderRadius: '6px', backgroundColor: '#FFFFFF', color: '#112235', cursor: 'pointer', outline: 'none' };
  return (
    <>
      ...
      <Divider />
      <SecRow label="Language"    desc="Preferred display language"        action={<select value={lang} onChange={e => setLang(e.target.value)} style={sel}><option>English</option><option>French</option></select>} />
      <SecRow label="Timezone"    desc="Used for timestamps and reports"   action={<select value={tz}   onChange={e => setTz(e.target.value)}   style={sel}><option>Africa/Accra (GMT+0)</option><option>UTC</option><option>Africa/Lagos (GMT+1)</option></select>} />
      <SecRow label="Date format"                                          action={<select value={fmt}  onChange={e => setFmt(e.target.value)}  style={sel}><option>DD/MM/YYYY</option><option>MM/DD/YYYY</option><option>YYYY-MM-DD</option></select>} last />
    </>
  );
}
...
function SectionSecurity({ data, onToast }) {
...
function SectionSessions() {
  ...
  const load = () => {
    setLoading(true);
    Promise.all([api.get('/auth/sessions'), api.get('/auth/login-history')])
  ...
  return (
    <>
      ...
      <p style={{ fontSize: '13px', color: '#6B7280', marginTop: '4px', marginBottom: '32px' }}>Manage your active sessions and login history</p>
      ...
      {loading ? <div style={{ fontSize: '13px', color: '#9CA3AF' }}>Loading…</div> : (
        sessions.length === 0
          ? <div style={{ fontSize: '13px', color: '#9CA3AF', padding: '16px 0' }}>No active sessions found.</div>
          : sessions.map(s => (
            <div key={s.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid #E5E7EB' }}>
              <div>
                <div style={{ fontSize: '13px', color: '#112235', fontWeight: '500' }}>{s.userAgent?.split(' ')[0] || 'Unknown device'}</div>
                <div style={{ fontSize: '12px', color: '#6B7280', marginTop: '2px' }}>{s.ipAddress} · {new Date(s.createdAt).toLocaleString()}</div>
              </div>
              {s.isCurrent
                ? <span style={{ fontSize: '11px', fontWeight: '600', color: '#059669', backgroundColor: '#D1FAE5', padding: '3px 8px', borderRadius: '4px' }}>Current</span>
                : <button onClick={() => revoke(s.id)} style={{ padding: '5px 12px', fontSize: '12px', fontWeight: '600', border: '1px solid rgba(220,38,38,0.3)', borderRadius: '6px', backgroundColor: '#FEE2E2', color: '#DC2626', cursor: 'pointer' }}>Revoke</button>
              }
            </div>
          ))
      )}
      ...
      <Divider />
      {history.map(h => (
        <div key={h.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #E5E7EB' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ fontSize: '11px', fontWeight: '600', padding: '2px 8px', borderRadius: '999px', backgroundColor: h.action === 'login' ? '#D1FAE5' : h.action === 'login_fail' ? '#FEE2E2' : '#F3F4F6', color: h.action === 'login' ? '#059669' : h.action === 'login_fail' ? '#DC2626' : '#6B7280' }}>
              {h.action.replace('_', ' ')}
            </span>
...
export default function ProfilePage() {
  ...
  const load = () => { api.get('/auth/profile').then(r => setData(r.data)).catch(console.error); };
  useEffect(() => { load(); }, []);
  ...
  const rightContent = () => {
    switch (activeKey) {
      case 'profile':       return <SectionProfile data={data} onRefresh={load} onToast={handleToast} goToSecurity={() => setActiveKey('security')} />;
      ...
      case 'security':      return <SectionSecurity data={data} onToast={handleToast} />;
